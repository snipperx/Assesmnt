...
function modalFormDataSubmit(
    ...
    $.ajax({
        ...
        success: function(success) {
            ...
            let successHTML = '<button type="button" id="close-invalid-input-alert" class="close" ' +
                'data-dismiss="alert" aria-hidden="true">&times;</button><h4><i class="icon fa fa-check"></i> ' +
                successMsgTitle + '</h4>';
            ...
            window.setTimeout(function() { myModal.find("#success-alert").fadeOut('slow'); },
                5000);
        ...
        error: function(xhr) {
            // console.log(xhr);
            if(xhr.status === 422) {
                const errors = xhr.responseJSON; //get the errors response data


                myModal.find('.form-group').removeClass('has-error'); //Remove the has error class to all form-groups

                let errorsHTML = '<button type="button" id="close-invalid-input-alert" class="close" ' +
                    'aria-hidden="true">&times;</button><h4><i class="icon fa fa-ban"></i> Invalid Input(s)!</h4><ul>';
